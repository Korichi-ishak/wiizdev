...
const RubiksCubeModel = forwardRef<any, any>((props, ref) => {
  ...
  const chromeMaterial = useMemo(() => ({
    color: theme === 'dark' ? '#D8973C' : '#273E47', // Primary color in dark mode, secondary in light mode
    metalness: theme === 'dark' ? 0.7 : 0.5,
    roughness: theme === 'dark' ? 0.3 : 0.4,
    clearcoat: theme === 'dark' ? 0.1 : 0.2,
    clearcoatRoughness: theme === 'dark' ? 0.1 : 0.15,
    reflectivity: theme === 'dark' ? 0.8 : 0.6,
    iridescence: theme === 'dark' ? 0.2 : 0.1,
    iridescenceIOR: 1.3,
    iridescenceThicknessRange: [100, 400],
    envMapIntensity: theme === 'dark' ? 10 : 6
  }), [theme]);
  ...
  return (
    <>
      <EnhancedSpotlight 
        ...
        color={theme === 'dark' ? '#D8973C' : '#273E47'}
        position={[3, 3, 2]}
        volumetric={true}
        opacity={1}
        penumbra={1}
      ...
      <ambientLight intensity={theme === 'dark' ? 0.3 : 0.5} color={theme === 'dark' ? '#273E47' : '#ffffff'} />
      <directionalLight 
        position={[-2, -2, 3]} 
...
export function Scene() {
  ...
  const backgroundColor = theme === 'dark' ? '#273E47' : '#ffffff';

  return (
    <div className="h-svh w-screen relative" style={{ backgroundColor }}>
      <Canvas
        shadows
        gl={{
          antialias: isDesktop,
